...
def graficas(palabrasInteres, tweetsSentimiento):
    ...
    tweetsSentimiento['clase'].value_counts().plot(kind='bar')
    ...
    dict_words_sentiment = {'word': [], 'sentiment': []}
    for index,line in tweetsSentimiento.iterrows():

        tw = line["tweet"].lower()
        for word in tw.split():
            if word in palabra:
                dict_words_sentiment['sentiment'].append(line["clase"])
                dict_words_sentiment['word'].append(word)
            print(dict_words_sentiment)
    df_word_sentiment = pd.DataFrame(data=dict_words_sentiment)
    sns.countplot(x='word', hue="sentiment", data=df_word_sentiment)
    plt.show(block=True)
    ...
    for i in range(0, len(claseSentimientos)):
        ...
        for j in range(0, len(p)):
            if (p[j] == "coronavirus" or p[j] == "covid-19" or p[j] == "covid19" or p[j] == "covid"):
                n += 1
        if (str(claseSentimientos[i]) == 'Negativo'):
            frecuencia[0] += n
        if (str(claseSentimientos[i]) == 'Neutro'):
            frecuencia[1] += n
        if (str(claseSentimientos[i]) == 'Positivo'):
            frecuencia[2] += n
        n = 0
